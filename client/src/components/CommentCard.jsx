...
const CommentCard = () => {
    ...
    return (
        <div className='mt-16'>
            ...
            {
                user &&
                <div className="w-full md:flex-row flex flex-col justify-center  gap-10 items-center">

                    {/* Left Content  */}
                    <div className="flex justify-center items-center">
                        <img src={feedbackImg} alt="" className='w-80 md:96' />
                    </div>

                    {/* Right Content  */}
                    <div className={`my-5 py-2  px-5 rounded-md ${theme === 'dark' ? 'border border-gray-600 ' : 'border border-gray-300'}`}>
                        <div className="flex gap-3 items-center">
                            <h1 className='text-sm font-semibold '>Share your feedback</h1>
                            <AiOutlineComment size={30} />
                        </div>
                        <form className={`  flex shadow-md flex-col  w-full px-1 md:px-5 my-5 py-5 rounded-md ${theme === 'dark' ? 'border border-gray-600 ' : 'border border-gray-300'}}`} onSubmit={commentSubmitHandle}>
                            ...
                            <span className={`text-sm mt-3 transition-all font-semibold ${commentData.length === 50 ? 'text-red-500 ' : 'text-green-500'}`}>{50 - commentData.length} characters left</span>
